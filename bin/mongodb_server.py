...
class mongodbMonitor(object):
		...
		try:
			...
			if user and password:
				...
				if not db_admin.authenticate(user,password):
	                		pass;
			conn.server_info()  # domore 如果有密码，是否可行
		except :
			e = sys.exc_info()[0]
			return e, None
	...
	def get_mongo_role(self, conn):
		...
		mongo_role = 1	
		conn.server_info() # 必须连接后,is_mongos判断才能生效
		if (conn.is_mongos):
			mongo_role = 2 
	...
	def  serverStatus(self,connection):
        ...
        def repl_status(self,connection):
	        replStatus = connection.admin.command("replSetGetStatus")
                ...
                window_multiple = 1   ##oplog.rs不一定已填满            
                ...
                oplog_window = round((oplog_tLast["ts"].time - oplog_tFirst["ts"].time)/3600.0,2) * window_multiple  # 当前已有的oplog记录时间差 * 倍数
	...
	def shard_status(self, conn):  
		...
		shards_dict = {}
       		...
       		elif balancer_doc["stopped"]:  # 如果是stopped, 设置有window    
               		shards_dict["shards_BalancerState"] = 0
       		else: #非stop状态
                	shards_dict["shards_BalancerState"] = 1

       		# shards_activeWindow metric,0: without setting, 1:setting 
       		# shards_activeWindow_start  metric,  { "start" : "23:30", "stop" : "6:00" } :  上传数据23.30 表示 23点30分 
